    ...
    return (
        <header className='flex items-center w-full p-4 md:py-2 md:px-6'>
            ...
            <div
                ...
                <img src={user?.user.imageURL} className='w-12 min-w-[44px] object-cover rounded-full shadow-lg' alt="" referrerPolicy='no-refer' />
                <div className='flex flex-col'>
                    <p className='text-textColor text-lg hover:text-headingColor font-semibold'>{user?.user?.name}</p>
                    <p className='flex items-center gap-2 text-xs text-gray-500 font-normal'>Premium Member. <FaCrown className='text-sm -ml-l text-yellow-500' /></p>
                </div>
                {isMenu && (
                    <motion.div
